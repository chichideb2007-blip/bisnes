...
def settings():
    if request.method == 'POST':
        supabase.table("companies").update({"store_name": request.form.get('store_name')}).eq("id", session['company_id']).execute()
...
def chat():
    ...
    response = client.models.generate_content(
        ...
        contents=f"أنت مساعد متجر ذكي. بيانات المنتجات: {products.data}. السؤال: {user_message}"
    )
