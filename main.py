...
# تعريف التطبيق في البداية (هذا يحل مشكلة NameError)
app = Flask(__name__)
...
def products():
    ...
    if request.method == 'POST':
        ...
        if image_file and image_file.filename != '':
            ...
            except Exception as e: return f"خطأ في رفع الصورة: {e}"
        new_prod = {
...
def search_products():
    ...
    comp_id = session['company_id']
    response = supabase.table("inventory").select("*").eq("company_id_text", comp_id).ilike("name", f"%{query}%").execute()
    return render_template('products.html', products=response.data or [])
