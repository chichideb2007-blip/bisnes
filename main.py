import os
import requests # المكتبة اللازمة للتنبيهات
...
def add_order():
    ...
    data = {
        ...
        "total_price": get_safe_float(price)
    }
    supabase.table("orders").insert(data).execute()
    
    # التنبيه: جلب بيانات الإعدادات وإرسال الرسالة
    s_res = supabase.table("settings").select("bot_token, telegram_chat_id").eq("manager_id", session['user']).maybe_single().execute()
    if s_res.data:
        token = s_res.data.get('bot_token')
        chat_id = s_res.data.get('telegram_chat_id')
