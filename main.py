...
# --- مسار الطلبات (مع الذكاء والأتمتة) ---
...
def orders():
    if request.method == 'POST':
        ...
        # 2. أتمتة خصم المخزن
        product = supabase.table("inventory").select("id, quantity").eq("name", product_name).execute()
        if product.data:
...
# --- مسار الإحصائيات (المنحنيات المتقدمة) ---
@app.route('/stats')
def stats():
    try:
        ...
        return f"خطأ في البيانات: {str(e)}"
